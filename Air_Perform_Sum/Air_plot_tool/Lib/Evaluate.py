from Lib.stonAvg import stonAvg

def Evaluate(obs_PH, sim_PH, obs_DA, sim_DA, stons, cats, var):
    ...
    for ston in stons:
#       print('Processing  '+area+'  '+var+'  '+ston)
        
        #(若為 MB 則使用 obs_PH 及 sim_PH 資料)
        for i in range(0, len(sim_PH), 24):
            obsMax = obs_PH[ston][i:i+24].max()
            simMax = sim_PH[ston][i:i+24].max()
