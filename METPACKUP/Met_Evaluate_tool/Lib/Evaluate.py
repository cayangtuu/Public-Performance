...
def evaluate(obsfile, simfile, ssList, var):
    ...
    if (var == 'WD'):
       ...
       for ston in ssList:
           ...
           if (sim_hr[ston] != 0):
              WNMB[ston] = str(WNMB[ston] * 100) + '%'
              WNME[ston] = str(WNME[ston] * 100) + '%' 
           
       if (sim_overal_hr != 0):
          WNMB['overal'] = str(round((WNMB['overal']/(360*sim_overal_hr)), 4) * 100) + '%'
          WNME['overal'] = str(round((WNME['overal']/(360*sim_overal_hr)), 4) * 100) + '%'

       if len(ssList) != 0:
          WNMB['Criteria'] = '10.0%/-10.0%'
          WNMB['合格率']   = str(round(good_WNMB/len(ssList), 2) * 100) + '%'
          WNMB['合格站數'] = good_WNMB
          WNME['Criteria'] = '30%'
          WNME['合格率']   = str(round(good_WNME/len(ssList), 2) * 100) + '%'
          WNME['合格站數'] = good_WNME
