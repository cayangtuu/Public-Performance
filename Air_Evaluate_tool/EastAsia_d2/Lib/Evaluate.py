    ...
    for ston in stons:
        ...
        for i in range(0, len(sim_PH)):
            if (float(obs_PH[ston][i]) != -999) and (float(sim_PH[ston][i]) != -999):
               ...
               sim_overal_hr  += 1
               MNB_Baby[ston] += (float(sim_PH[ston][i]) - float(obs_PH[ston][i])) / float(obs_PH[ston][i])
               MNE_Baby[ston] += abs(float(sim_PH[ston][i]) - float(obs_PH[ston][i])) / float(obs_PH[ston][i])
